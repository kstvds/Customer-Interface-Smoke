...
/* #######  Test for Transfer Searching #########
######  Scenario Logs In, Searches for a specified transfer and then verifies the search ##### */

public class Transfer_Search {
	...
	Logger logger = Logger.getLogger("Transfer_Search");

	/* ####### Passing browser as parameters in test ######### **/

	@Test
	@Parameters({ "browsername" })
	public void TransferSearch(String browsername) throws Exception {
		test = rep.startTest("Transfer Search");
		...
		try {
			...
			Thread.sleep(2000);
			obj.Takesnap(driverqa, Config.SnapShotPath() + "/Transfer/Transfer_Search/Log-In.jpg");

		} catch (Throwable e) {

			obj.Takesnap(driverqa, Config.SnapShotPath() + "/Transfer/Error/Transfer_Search/Log-In.jpg");
			test.log(LogStatus.FAIL, "Login");
			errorpath = Config.SnapShotPath() + "/Transfer/Error/Transfer_Search/Log-In.jpg";
		...
		/* ####### Applying filters and searching for Transfer ######### **/
		...
		try {
			...
			driverqa.findElement(Tour.TourdestID).sendKeys(excel.getData(0, 34, 3));
			Thread.sleep(3000);
			action.sendKeys(Keys.ARROW_DOWN).build().perform();
			action.sendKeys(Keys.ENTER).build().perform();
			driverqa.findElement(Transfer.transfername).sendKeys(excel.getData(0, 34, 1));
			Thread.sleep(4000);
			action.sendKeys(Keys.ARROW_DOWN).build().perform();
			action.sendKeys(Keys.ENTER).build().perform();
			 ...
			 */
			Thread.sleep(2000);
			obj.Takesnap(driverqa, Config.SnapShotPath() + "/Transfer/Transfer_Search/Filters.jpg");
			String expectedresult = excel.getData(0, 34, 1);
			...
			Thread.sleep(2000);
			obj.Takesnap(driverqa, Config.SnapShotPath() + "/Transfer/Transfer_Search/Search-Result.jpg");
			...
			obj.Takesnap(driverqa, Config.SnapShotPath() + "/Transfer/Error/Transfer_Search/Search-Result.jpg");
			errorpath = Config.SnapShotPath() + "/Transfer/Error/Transfer_Search/Search-Result.jpg";
			logger.info(e.getMessage());
			test.log(LogStatus.FAIL, e.getMessage());
			rep.endTest(test);
			rep.flush();
