...
/* #######  Test for Private Transfer booking #########
######  Scenario Logs In, Books a specified Private Transfer  ##### */

public class Transfer_Booking_Private {
	...
	Logger logger = Logger.getLogger("Transfer_Booking_Private");

	/* ####### Passing browser as parameters in test ######### **/

	@Test
	@Parameters({ "browsername" })
	public void TransferBookingPrivate(String browsername) throws Exception {
		test = rep.startTest("Transfer Booking Private");
		...
		try {
			...
			String ExpectedTransferName = excel.getData(0, 34, 1);
			...
			Assert.assertTrue(ActualStatus.equalsIgnoreCase(ExpectedStatus));
			Assert.assertTrue(ActualTransferName.equalsIgnoreCase(ExpectedTransferName));
