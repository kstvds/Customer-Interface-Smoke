...
public class Accomodation_Booking_Documents {
	public WebDriver driverqa;
	ExtentTest test;
	String errorpath;
	String Roomtype;
	ExcelDataConfig excel;
	String ExpectedBookingDocumentStatus;
	String ActualBookingDocumentStatus;
	String ExpectedBookingDocumentCheckIn;
	String ActualBookingDocumentCheckIn;
	String ExpectedBookingDocumentCheckOut;
	String ActualBookingDocumentCheckOut;
	String ExpectedBookingDocumentName;
	String ActualBookingDocumentName;
	String ExpectedBookingDocumentNoOfAdults;
	String ActualBookingDocumentNoOfAdults;
	String ExpectedBookingProformaHotelName;
	String ActualBookingProformaHotelName;
	Configuration Config = new Configuration();
	Takescreenshot obj = new Takescreenshot();
	ExtentReports rep = ExtentManager.getInstance();
	LoginPage login = new LoginPage();
	Logger logger = Logger.getLogger("Accomodation_Booking_Documents");

	/* ####### Passing browser as parameters in test ######### **/

	@Test
	@Parameters({ "browsername" })
	public void AccomodationBookingDocuments(String browsername) throws Exception {
		test = rep.startTest("Accomodation Booking Documents");
		...
		try {
			logger.info("Verifing Booking document");
			test.log(LogStatus.INFO, "Verifing Booking document");
			driverqa.findElement(Others.BookingDocuments).click();
			wait.until(ExpectedConditions.visibilityOfElementLocated(Others.BookingDocumentStatus));
			ExpectedBookingDocumentStatus = "Confirmed";
			ActualBookingDocumentStatus = driverqa.findElement(Others.BookingDocumentStatus).getText();
			ExpectedBookingDocumentCheckIn = excel.getData(0, 65, 1);
			ExpectedBookingDocumentCheckOut = excel.getData(0, 65, 2);
			ExpectedBookingDocumentName = excel.getData(0, 21, 1);
			ExpectedBookingDocumentNoOfAdults = "2 Adults";
			ActualBookingDocumentCheckIn = driverqa.findElement(Others.BookingDocumentCheckin).getText();
			ActualBookingDocumentCheckOut = driverqa.findElement(Others.BookingDocumentCheckOut).getText();
			ActualBookingDocumentName = driverqa.findElement(Others.BookingDocumentPassengerName).getText();
			ActualBookingDocumentNoOfAdults = driverqa.findElement(Others.BookingDocumentNoOfAdults).getText();
			Assert.assertTrue(ActualBookingDocumentStatus.contains(ExpectedBookingDocumentStatus));
			Assert.assertTrue(ActualBookingDocumentCheckIn.contains(ExpectedBookingDocumentCheckIn));
			Assert.assertTrue(ActualBookingDocumentCheckOut.contains(ExpectedBookingDocumentCheckOut));
			Assert.assertTrue(ActualBookingDocumentName.contains(ExpectedBookingDocumentName));
			Assert.assertTrue(ActualBookingDocumentNoOfAdults.contains(ExpectedBookingDocumentNoOfAdults));
			Thread.sleep(2000);
			obj.Takesnap(driverqa, Config.SnapShotPath()
					+ "/Accomodation/Accommodation_Book_DOTW_Room_Outside_Cancellation/Booking-Details2.jpg");
			test.log(LogStatus.PASS, "Verification of Booking document");
			logger.info("Verified Booking document");
			action.sendKeys(Keys.ESCAPE).build().perform();
			logger.info("Verifing Proforma document");
			test.log(LogStatus.INFO, "Verifing Proforma document");
			Thread.sleep(2000);
			wait.until(ExpectedConditions.elementToBeClickable(Others.BookingProforma));
			driverqa.findElement(Others.BookingProforma).click();
			wait.until(ExpectedConditions.visibilityOfElementLocated(Others.BookingProformaHotelName));
			ExpectedBookingProformaHotelName = excel.getData(0, 12, 1);
			ActualBookingProformaHotelName = driverqa.findElement(Others.BookingProformaHotelName).getText();
			ActualBookingDocumentCheckIn = driverqa.findElement(Others.BookingProformaDate).getText();
			System.out.println(ActualBookingProformaHotelName);
			Assert.assertTrue(ActualBookingProformaHotelName.equalsIgnoreCase(ExpectedBookingProformaHotelName));
			
			System.out.println(ActualBookingDocumentCheckIn);
			System.out.println(ExpectedBookingDocumentCheckIn);
			System.out.println(ExpectedBookingDocumentCheckOut);
			Assert.assertTrue(ActualBookingDocumentCheckIn.contains(ExpectedBookingDocumentCheckIn));
			Assert.assertTrue(ActualBookingDocumentCheckIn.contains(ExpectedBookingDocumentCheckOut));
			test.log(LogStatus.PASS, "Verification of Proforma document");
			logger.info("Verified Proforma document");
		} catch (Throwable e) {
			test.log(LogStatus.FAIL, "Hotel Booking Document Verification");
			obj.Takesnap(driverqa, Config.SnapShotPath()
					+ "/Accomodation/Error/Accommodation_Book_DOTW_Room_Outside_Cancellation/Booking.jpg");
			errorpath = Config.SnapShotPath()
					+ "/Accomodation/Error/Accommodation_Book_DOTW_Room_Outside_Cancellation/Booking.jpg";
